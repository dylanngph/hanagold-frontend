...
const Stake = ({farm, lpTokenName, userData}) => {
  ...
  const {onApprove} = useApprove(lpAddress, address.masterChefDfl);
  ...
  const {onStake} = useStake(pid, address.masterChefDfl);
  const {onUnstake} = useUnstake(pid, address.masterChefDfl);
