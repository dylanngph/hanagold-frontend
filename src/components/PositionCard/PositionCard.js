...
export default function FullPositionCard({ pair, ...props }) {
  ...
  return (
    <div color="primary" className="p-3 cursor-pointer mb-2 rounded-lg border-solid border-2 border-gray-400" {...props}>
          ...
          {userPoolBalance && JSBI.greaterThan(userPoolBalance.raw, BIG_INT_ZERO) && (
            <div className="mt-2">
              <Link to={`/remove/${currencyId(currency0)}/${currencyId(currency1)}`}>
                <Button color="blue" className="w-full mb-3 mt-5 flex align-center text-black">
                  Remove
                </Button>
              </Link>
              <Link to={`/add/${currencyId(currency0)}/${currencyId(currency1)}`}>
                <div className="w-full flex items-center text-primary justify-center p-3">
                  <Plus />
